...
class UpiTransactions:
    ...
    def initiate_transaction(self, sender_upi_id: str, receiver_upi_id: str, 
        ...
        if not validate_upi_id(sender_upi_id):
            return {"status": "FAILED", "error": "Invalid sender UPI ID format"}
        ...
        try:
            ...
            if not sender:
                return {"status": "FAILED", "error": "Sender UPI ID not found or inactive"}
                
            # Get receiver's UPI account
            cursor.execute(
                """
            ...
            if not receiver:
                return {"status": "FAILED", "error": "Receiver UPI ID not found or inactive"}
            ...
            logger.error(f"Error in UPI transaction: {e}")
...
def main():
    print("UPI Transactions App Running...")
