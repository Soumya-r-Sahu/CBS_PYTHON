...
try:
    ...
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))  # Add project root to path    # Import everything from the main config file
try:
    from config import *
except ImportError:
    ...
    UPI_CONFIG = {
        ...
        'api_key': os.environ.get('CBS_UPI_API_KEY', 'default_key')
    }
    
    # ATM service configurations
    ATM_CONFIG = {
        'max_withdrawal_limit': float(os.environ.get('CBS_ATM_MAX_WITHDRAWAL_LIMIT', '10000')),
    ...
    EMAIL_CONFIG = {
        ...
        'user': os.environ.get('CBS_EMAIL_USER', 'user@example.com'),
        'password': os.environ.get('CBS_EMAIL_PASSWORD', 'password')
    }
    ...
    UPI_PROVIDER = "default"
    UPI_API_KEY = "default_key"
    ATM_MAX_WITHDRAWAL_LIMIT = 10000
    ATM_PIN_ATTEMPTS = 3
